...
def create_mutation(sequence_id=None, mutation_type=None, position=None,
                    original_base=None, mutated_base=None,
                    mutation_sequence=None, disease_id=None):
    conn = get_connection()
    if not conn: return None
    try:
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO Mutations
              (sequence_id, mutation_type, position, original_base,
               mutated_base, mutation_sequence, disease_id)
            VALUES (%s,%s,%s,%s,%s,%s,%s)
        """, (sequence_id, mutation_type, position, original_base,
              mutated_base, mutation_sequence, disease_id))
        conn.commit()
...
def fetch_all_mutations():
    """Used by analyzer.py — returns mutations joined with disease and gene info."""
    conn = get_connection()
    if not conn: return []
    try:
        cur = conn.cursor(dictionary=True)
...
def get_all_mutations():
    ...
    try:
        cur = conn.cursor(dictionary=True)
        cur.execute("""
            SELECT m.*, d.disease_name, d.severity,
                   s.gc_content, s.sequence_length, g.gene_name
            FROM Mutations m
            LEFT JOIN Diseases  d ON m.disease_id  = d.disease_id
            LEFT JOIN Sequences s ON m.sequence_id  = s.sequence_id
            LEFT JOIN Genes     g ON s.gene_id      = g.gene_id
...
def get_mutations_paginated(page=1, per_page=20, search='', risk='', category=''):
    ...
    try:
        ...
        where, params = [], []
        if search:
            where.append("""(m.mutation_type LIKE %s OR m.original_base LIKE %s
                             OR m.mutated_base LIKE %s OR d.disease_name LIKE %s)""")
            s = f"%{search}%"
            params += [s, s, s, s]
        clause = ("WHERE " + " AND ".join(where)) if where else ""
        cur.execute(f"""
            SELECT COUNT(*) as cnt FROM Mutations m
            LEFT JOIN Diseases d ON m.disease_id = d.disease_id
            {clause}
        """, params)
        total = cur.fetchone()['cnt']
        ...
        cur.execute(f"""
            ...
            LEFT JOIN Genes     g ON s.gene_id      = g.gene_id
            {clause}
            ORDER BY m.mutation_id DESC LIMIT %s OFFSET %s
        """, params + [per_page, offset])
        return cur.fetchall(), total
    finally:
        conn.close()
...
def get_mutation_categories():
    ...
    try:
        cur = conn.cursor()
        cur.execute("""
            SELECT DISTINCT mutation_type FROM Mutations
            WHERE mutation_type IS NOT NULL ORDER BY mutation_type
        """)
        return [r[0] for r in cur.fetchall()]
    finally:
        conn.close()
        ...
        cur.execute("""
            SELECT p.*,
              (SELECT COUNT(*) FROM Samples s
               WHERE s.patient_id = p.patient_id) as sample_count,
              (SELECT COUNT(*) FROM Patient_Analysis pa
               JOIN Samples s ON pa.sample_id = s.sample_id
               WHERE s.patient_id = p.patient_id) as analysis_count
            FROM Patients p ORDER BY p.date_registered DESC
...
def get_samples_by_patient(patient_id):
    ...
    try:
        cur = conn.cursor(dictionary=True)
        cur.execute(
            "SELECT * FROM Samples WHERE patient_id=%s ORDER BY collection_date DESC",
            (patient_id,)
        )
        return convert_datetimes_list(cur.fetchall())
    finally:
...
def save_result(patient_name, dna_sequence, results_json,
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO analysis_results
              (patient_id, patient_name, dna_sequence, filename,
               results, notes, analyzed_by)
            VALUES (%s,%s,%s,%s,%s,%s,%s)
        """, (patient_id, patient_name, dna_sequence[:500],
              filename, results_json, notes, analyzed_by))
        conn.commit()
